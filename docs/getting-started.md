...
**You'll finish with:** the app serving locally, a few papers ingested, and a grounded,
cited answer to your first question.
...
- **A cloud provider (Anthropic/Gemini) or a different local port.** See
...
You should see the agent's **trace** (Thought → Action → Observation) as it searches, then
a streamed answer with `[r1]`, `[r2]` citations. Click a citation — it opens the paper on
the **Papers** page with the cited passage highlighted.

If the agent answers with no citations for a paper question, retrieval returned nothing —
confirm on the Admin page that ingestion actually finished.
...
- Change models, paths, or the paper list → [Configuration & commands](configuration.md).
- Add a paper or swap a backend → [How-to guides](how-to.md).
- Understand how retrieval and the agent work → [Architecture](architecture.md).
