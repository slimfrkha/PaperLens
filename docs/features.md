...
- **"Broaden recall per paper" toggle (Ask mode only)** — runs `search_papers` recall once per paper and pools candidates flat before reranking instead of once over the whole scope, sticky per message (not locked mid-conversation like the filters above); reloading or switching to a conversation restores the toggle to its last message's actual value rather than resetting it, and it's off by default for a brand-new chat; hidden (and never sent) under Compare mode, since Compare's per-paper sub-runs already search one paper at a time (`web/src/pages/ChatPage.tsx`, `src/server/agent.py`, `src/server/chats.py`)

### Chat — Compare mode

- **Ask/Compare primary switch** — segmented control choosing the turn's answer shape: one combined answer over the shared retrieval pool (Ask) vs. a guaranteed per-paper search+answer synthesized into one comparative answer (Compare); sticky per message, resets on New chat / restores from the conversation's last-used mode on reload, same semantics the secondary knob already has (`web/src/pages/ChatPage.tsx`)
