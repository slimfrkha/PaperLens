...
dev:
	uv run paperlens-serve --config_path $(CONFIG) & npm --prefix web run dev; kill %1 2>/dev/null || true

# Ingest configured papers not yet in the DB.
ingest:
	uv run paperlens-ingest --config_path $(CONFIG)

# Backend only.
serve:
	uv run paperlens-serve --config_path $(CONFIG)
