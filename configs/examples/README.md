...
The base `uv sync` installs every supported provider; there are no provider-specific extras.
...
| [`anthropic.yaml`](anthropic.yaml) | local HF | Anthropic (Claude) | `ANTHROPIC_API_KEY` |
| [`gemini.yaml`](gemini.yaml) | Google Gemini | Gemini | `GEMINI_API_KEY` |
...
[docs/configuration.md](../../docs/configuration.md). Relative paths are anchored to the
nearest `pyproject.toml` ancestor of the config file (the PaperLens project root for these
templates), not to the current working directory.
